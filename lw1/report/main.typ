...
Даны 6 вершин $v_i$ и 2 точки $a$ и $b$.

#let dataset = csv("csv/dataset.csv")

#figure(
  caption: [
    Точки, в соответствие с вариантом.
  ],
  table(
    columns: (25%, 25%, 25%, 25%),
    align: center,
...
\
== Теория
#underline[Определение] (Норма в банаховом пространстве). Пусть $E$ -- вещественнозначное линейное пространство. Отображение $||dot||: E -> RR$ такое, что $forall x,y,z in E$ выполняются условия
+ $||x|| >= 0$, $||x|| = 0$, только если $x = 0$: положительная определённость;
+ $||lambda x|| = |lambda| dot ||x|| forall lambda in F$: однородность нормы;
+ $||x + y|| <= ||x|| + |y||$: неравенство треугольника.
называется *нормой*.

#underline[Определение] (Внутренняя точка). Точка $x in X$ называется внутренней точкой $W$, если существует окрестность этой точки целиком содержащаяся в $W$.

#underline[Алгоритм] (Проверка выпуклости многогранника). Пусть $X$ -- линейное нормированное пространство. Множество $W subset X$ называется *выпуклым*, если для любых двух точек $x, y in W$ отрезок $(lambda x + (1 - lambda)y | lambda in [0, 1])$ целиком принадлежит $W$.

#underline[Теорема] (Минковского). Пусть $W in X$ - выпуклое множество, а 0 является его внутренней точкой и точкой симметрии. Тогда $||x||_W = inf {lambda : x / lambda in W, lambda > 0}$ задаёт норму в пространстве $X$.
...
После этого были заданы грани многогранника в первой квадранте, а затем отображены на остальные квадранты относительно соответствующих осей.
...
Пусть $arrow(p_1)$, $arrow(p_2)$, $arrow(p_3)$ -- точки в $RR^3$, для которых строится поверхность. Для получения уравнений поверхностей использовалось свойство нормали к поверхности, которое гласит, что нормаль $arrow(n) = [(arrow(p_2) - arrow(p_1)) times (arrow(p_3) - arrow(p_1))]$ равна вектору $(A, B, C)$, где $A$, $B$, $C$ --- соответствующие коэффициенты плоскости. $D$ вычисляется из уравнения $A x + B y + C z + D = 0$ подстановкой точки $arrow(p_1)$.
...
При более детальном рассмотрении результатов, представленных в @surface_equasions видно, что все точки, не лежащие на рассматриваемой плоскости $s_i$ лежат от неё по одну сторону (так как значения $f$, при применении к точкам $v_j in.not s_i$ имеют один знак), т.е. многогранник является выпуклым в положительной квадранте. Также, в силу симметрии плоскостей в разных квадрантах относительно соответствующих осей, многогранник является выпуклым.
...
Для вычисления биортогонального базиса $(arrow(b_1), arrow(b_2), arrow(b_3))$ для базиса $(arrow(a_1), arrow(a_2), arrow(a_3))$ используется векторное произведение векторов исходного базиса
...
Как видно из таблицы, луч в каждую из искомых точек попадает в грань $s_4$. Тогда искомые нормы равны
